...
/**
 * \struct Mesh
 * \brief structure that contains all the information of the mesh that this necessary for the DG method
 */
struct Mesh
{
    ...
    //      [end maybe not]
    std::vector<int> elementTags; /*!< [e1, e2, e3, ...]: concatenation of element tags [maybe not] */
    //std::vector<std::vector<int>> elementTags;
    std::vector<int> nodeTags; /*!< [n1a, n1b, n2a, n2b, ...]: concatenation of the edges nodes, where each node appears only once ("reduced" list of nodes) */

    std::vector<std::vector<int>> parentElement;  /**< [[e1, e2], [e1], ...]: vector such that
                                                       parentElement[i] is associated to the i-th edge contained in
                                                       nodeTags (i.e., the node tags of the edge are (nodesTags[2*i],
                                                       nodesTags[2*i+1]) */

    std::vector<std::vector<double>> normalVector;/**< [[n1x, n1y, n2x, n2y], [n2x, n2y], ...]: vector that
    ...
    std::vector<double> jacElement;/**< [jac1gp1, jac1gp2, jac2gp1, ...]: jacobian of each element, evaluated at the Gauss points*/
