		...
		for(size_t elm = 0 ; elm < entity.elements.size() ; elm++)
		{
			...
			for(unsigned short unk = 0 ; unk < solverParams.nUnknowns ; ++unk)
            {
                partialField.partialIu[unk]
                	.resize(mesh.elementProperties
                			.at(entity.elements[elm].elementTypeHD).nSF);
			...
			for(unsigned int s = 0 ; s < nSigma ; ++s)
			{
				...
				for(unsigned int j = 0 ; 
						j < entity.elements[elm].edges[s].offsetInElm.size() ; ++j)
				{
					// global index of the current node
					unsigned int indexJ = entity.elements[elm].offsetInU
						 + entity.elements[elm].edges[s].offsetInElm[j];
					...
					if (entity.elements[elm].edges[s].edgeInFront.first == -1)
					{
						...
						ibc boundary = solverParams.boundaryConditions
								.at(entity.elements[elm].edges[s].bcName);

						boundary.ibcFunc(partialField.uAtBC, 
								entity.elements[elm].edges[s].nodeCoordinate[j], t,
					...
					else // general case
					{
					    // global index of the node "in front"
                       	unsigned int indexFrontJ =
                       				entity
                       					.elements[entity.elements[elm].edges[s]
                       						.edgeInFront.first].offsetInU
                       				+ entity
                       					.elements[entity.elements[elm].edges[s]
                       								.edgeInFront.first]
                       					.edges[entity.elements[elm].edges[s]
                       							.edgeInFront.second]
                       					.offsetInElm[entity.elements[elm]
                       									.edges[s]
				...
				for(unsigned short unk = 0 ; unk < solverParams.nUnknowns ; ++unk)
                {
                    partialField.partialIu[unk] +=
                    	entity.elements[elm].edges[s].determinantLD[0]*(
							entity.elements[elm].edges[s].normal[0]*entity
								.elements[elm].dM[s]*partialField.g[0][unk]
							+ entity.elements[elm].edges[s].normal[1]*entity
			...
			for(unsigned short unk = 0 ; unk < field.Iu.size() ; ++unk)
            {
                for(unsigned int j = 0 ; 
                	j < mesh.elementProperties
                		.at(entity.elements[elm].elementTypeHD).nSF ; ++j)
                {
                    field.Iu[unk][entity.elements[elm].offsetInU + j]
